...
import { Car, LayoutDashboard, Users, Settings, MapPin, Star, Package, UserCircle, X, Menu } from 'lucide-react'
  ...
  return (
    <>
      <header className="lg:hidden h-14 border-b border-border/40 bg-white/80 dark:bg-background/80 backdrop-blur-md flex items-center justify-between px-4 sticky top-0 z-30">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" className="h-8 text-xs shadow-sm">Soporte</Button>
      ...
      <div className={`lg:hidden fixed top-0 left-0 h-full w-72 bg-white dark:bg-background border-r border-border/40 z-50 flex flex-col shadow-2xl transition-transform duration-300 ease-in-out ${open ? 'translate-x-0' : '-translate-x-full'}`}>
